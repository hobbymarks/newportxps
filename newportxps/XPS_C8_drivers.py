...
class XPS:
    ...
    def ControllerStatusGet(self, socketId=None):
        error, returnedString = self.Send(
            socketId=socketId, cmd="ControllerStatusGet(int *)", check=True
        )
        i, j, retList = 0, 0, [error]
        while (i + j) < len(returnedString) and returnedString[i + j] != ",":
            j += 1
    ...
    def TimerSet(self, socketId, TimerName, FrequencyTicks):
        return self.Send(
            socketId, "TimerSet(%s, %s)" % (TimerName, str(FrequencyTicks))
        )
    ...
    def GatheringExternalDataGet(self, socketId, IndexPoint):
        return self.Send(
            socketId, "GatheringExternalDataGet(%s, char *)" % str(IndexPoint)
        )
    ...
    def GroupInitializeWithEncoderCalibration(self, socketId, GroupName):
        return self.Send(
            socketId, "GroupInitializeWithEncoderCalibration(%s)" % GroupName
        )
    ...
    def GroupStatusStringGet(self, socketId, GroupStatusCode):
        return self.Send(
            socketId, "GroupStatusStringGet(%s, char*)" % str(GroupStatusCode)
        )
    ...
    def PositionerCorrectorPIDFFAccelerationSet(
        ...
        command = (
            ...
            + str(KForm)
            + ","
            + str(FeedForwardGainAcceleration)
            + ")"
        )
        return self.Send(socketId, command)
    ...
    def PositionerCorrectorTypeGet(self, socketId, PositionerName):
        return self.Send(
            socketId, "PositionerCorrectorTypeGet(%s, char *)" % PositionerName
        )
    ...
    def PositionerErrorStringGet(self, socketId, PositionerErrorCode):
        return self.Send(
            socketId, "PositionerErrorStringGet(%s, char *)" % str(PositionerErrorCode)
        )
    ...
    def PositionerHardwareStatusStringGet(self, socketId, PositionerHardwareStatus):
        return self.Send(
            socketId,
            "PositionerHardwareStatusStringGet(%s, char *)"
    ...
    def PositionerPositionCompareAquadBPrescalerSet(
        self, socketId, PositionerName, PCOInterpolationFactor
    ):
        command = (
            "PositionerPositionCompareAquadBPrescalerSet("
            + PositionerName
            + ","
            + str(PCOInterpolationFactor)
            + ")"
        )
        return self.Send(socketId, command)
    ...
    def PositionerPositionCompareAquadBPrescalerGet(self, socketId, PositionerName):
        command = (
            "PositionerPositionCompareAquadBPrescalerGet("
            + PositionerName
            + ",double *)"
        )
        error, returnedString = self.Send(socketId, command)
    ...
    def PositionerDACOffsetDualSet(
        ...
        command = (
            "PositionerDACOffsetDualSet("
            + PositionerName
            + ","
            + str(PrimaryDACOffset1)
            + ","
            + str(PrimaryDACOffset2)
            + ","
            + str(SecondaryDACOffset1)
            + ","
            + str(SecondaryDACOffset2)
            + ")"
        )
        return self.Send(socketId, command)
    ...
    def MultipleAxesPTVerification(self, socketId, GroupName, TrajectoryFileName):
        command = (
            "MultipleAxesPTVerification(" + GroupName + "," + TrajectoryFileName + ")"
        )
        return self.Send(socketId, command)
    ...
    def MultipleAxesPTExecution(
        self, socketId, GroupName, TrajectoryFileName, ExecutionNumber
    ):
        command = (
            "MultipleAxesPTExecution("
            + GroupName
            + ","
            + TrajectoryFileName
            + ","
    ...
    def OptionalModuleExecute(self, socketId, ModuleFileName, TaskName):
        return self.Send(
            socketId, "OptionalModuleExecute(%s, %s)" % (ModuleFileName, TaskName)
        )
    ...
    def EEPROMCIESet(self, socketId, CardNumber, ReferenceString):
        return self.Send(
            socketId, "EEPROMCIESet(%s, %s)" % (str(CardNumber), ReferenceString)
        )
    ...
    def EEPROMDriverSet(self, socketId, PlugNumber, ReferenceString):
        return self.Send(
            socketId, "EEPROMDriverSet(%s, %s)" % (str(PlugNumber), ReferenceString)
        )

    # EEPROMINTSet :  Set INT EEPROM reference string
    def EEPROMINTSet(self, socketId, CardNumber, ReferenceString):
        return self.Send(
            socketId, "EEPROMINTSet(%s, %s)" % (str(CardNumber), ReferenceString)
        )
    ...
    def HardwareDriverAndStageGet(self, socketId, PlugNumber):
        return self.Send(
            socketId, "HardwareDriverAndStageGet(%s, char *, char *)" % str(PlugNumber)
        )
